...
- **Some content can't be reached by the scrape** and requires IIE to supply source files: gated PDFs (Regional Fact Sheets, Global Guide) and portal-only content (Region landing pages, Webinars, Campus News). Source comes from Drive/Box, the file directory, or a REAC-login export.
- **The bigger concern is gated content with no source file.** Without a D7 DB dump (B1), several categories can't be migrated at all — the 12 Region landing pages (L12), portal-only Campus News (L3), Adviser Profiles (L6), Webinars (L8), and the \~778 orphaned `/node/N` URLs (L7) would need manual re-authoring on the new site. Rough combined volume: a few hundred pages, but accurate sizing requires portal access.
